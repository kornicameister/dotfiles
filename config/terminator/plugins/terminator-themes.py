...
class TerminatorThemes(plugin.Plugin):
    ...
    def configure(self, widget, data=None):
        ...
        main_container.pack_start(
            self._create_themes_grid(ui), True, True, 0
        )  #Left column
        main_container.pack_start(
    ...
    def _create_filter_widgets(self, ui):
        ...
        search_entry.connect(
            "search-changed", self.on_theme_search_changed, ui
        )
    ...
    def on_install(self, button, data):
        ...
        template_data = self.config_base.profiles[self.inherits_config_from
                                                  ].copy()
...
class ThemePreview(Gtk.VBox):
    ...
    def update_prompt_line_colors(self, palette):
        ...
        self.prompt_line["prompt"].modify_fg(
            0, color=Gdk.color_parse(palette[6])
        )
